<x-layout>
    <div class="container py-md-5 container--narrow">
      <h2>
        ...
        @if (!sharedData['currentlyFollowing'] AND auth()->user()->username != sharedData['username']) {{-- checks if not currently following and not looking at your own profile --}}
        ...
        @if(auth()->user()->username == $sharedData['username']) {{-- if looking at user's own profile, change avatar button appears --}}
      ...
      <div class="profile-nav nav nav-tabs pt-2 mb-4">
        <a href="/profile/{{$sharedData['username']}}" class="profile-nav-link nav-item nav-link {{ Request::segment(3) == "" ? "active" : "" }}">Posts: {{$sharedData['postCount']}}</a> {{-- dynamically selects the active tab based on the URL --}}
